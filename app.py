...
def _bar_chart(
    ...
    df = pd.DataFrame({"model": y_labels, "value": x_vals, "color": colors})
    return (
        alt.Chart(df, title=title)
        .mark_bar()
        .encode(
            x=alt.X("value:Q", title=x_title),
            y=alt.Y("model:O", sort=None, title="",
                    axis=alt.Axis(labelLimit=220)),
            color=alt.Color(
                "model:N",
                scale=alt.Scale(domain=y_labels, range=colors),
                legend=None,
            ),
            tooltip=[
        ...
        .properties(height=max(120, len(y_labels) * 48))
    )
...
with tab_leader:
    if not st.session_state.bench_results:
        ...
        if not cat_avg.empty:
            model_names_ordered = cat_avg["model_name"].unique().tolist()
            model_colors_ordered = [color_map.get(n, "#999") for n in model_names_ordered]
            grouped = (
                alt.Chart(cat_avg, title="Avg Latency (ms) by Category")
                .mark_bar()
                .encode(
                    x=alt.X("category:O", title="",
                            axis=alt.Axis(labelAngle=-30, labelLimit=160)),
                    y=alt.Y("latency_ms:Q", title="Avg Latency (ms)"),
                    color=alt.Color(
                        "model_name:N",
                        scale=alt.Scale(
                            domain=model_names_ordered,
                            range=model_colors_ordered,
                        ),
                    ...
                    tooltip=[
                        ...
                        alt.Tooltip("latency_ms:Q", title="Avg Latency (ms)", format=".0f"),
                    ],
                )
